...
def get_list_selection(lists): # Gets the lists that analysis is needed on from the user, seperated by a comma.
    try:
        ...
        list_selection = response.replace(' ', '').split(',')
        print('')
...
def build_cards(cards_by_list): # Creates a Card_Object for each of the cards in the dictionary passed in, and assigns it to the list_objects card_objects value.
    for i in cards_by_list:
        ...
        try: 
            ...
            write_to_log('error', f'Unable to build cards for {i.name}')
    return

"""DATA ANALYSIS FUNCTIONS"""
def analyze_cards(cards_by_list):
    reports = []
    try:
        for i in cards_by_list:
    ...
    try:
        for i in report.TeamMemberStats:
            if i.name in labels:
                ...
                if 'Blocked' in labels: #  and any(entry in labels for entry in ['Retested', 'Failed', 'Passed']) I dont remember why this is here, but it doesnt seem to do anything other than break this function.
...
def program():
    ...
    reports = analyze_cards(cards_by_list)
